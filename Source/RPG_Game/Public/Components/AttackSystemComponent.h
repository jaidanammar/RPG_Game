...
enum class ERPGAttackInputType : uint8
{
    ...
    Special UMETA(Hidden),
    Charged UMETA(Hidden),
    Aerial UMETA(Hidden),
    LightSlash UMETA(Hidden),
    LightStab UMETA(Hidden)
};

UENUM(BlueprintType)
enum class ERPGAttackContinuationMode : uint8
{
    ComboLinks UMETA(DisplayName = "Combo Links"),
    ContextualSelection UMETA(DisplayName = "Contextual Selection")
};
...
public:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Flow")
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Input", meta=(ClampMin="0.0", ClampMax="1.0"))
    float PrimaryHeavyHoldThreshold = 0.25f;
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Movement")
    bool bPreserveMomentumWhenMovementInputHeld = true;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Movement")
    bool bApplyPostAttackMovementLock = true;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Movement")
    bool bSkipPostAttackMovementLockWhenMovementInputHeld = true;
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Animation")
    bool bUseMontageBlendOutForComboFlow = true;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Animation")
    bool bStopAttackMontageOnComboEnd = true;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Animation", meta=(ClampMin="0.0", ClampMax="0.2"))
    float StopAttackMontageBlendOutTime = 0.03f;
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Trace")
    FName DefaultTraceEndComponentName = TEXT("WeaponTraceEnd");

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Trace", meta=(ClampMin = "0.0"))
    float TraceFallbackForwardDistance = 120.0f;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Attack|Trace")
    FVector TraceFallbackLocalStartOffset = FVector(0.0f, 0.0f, 50.0f);
...
private:
    ...
    bool bPrimaryAttackConsumedByHold = false;
    float PrimaryAttackHeldDuration = 0.0f;
    ...
    bool HasAttackStartForInputType(ERPGAttackInputType InputType) const;
    ERPGAttackInputType ResolvePrimaryAttackInputType() const;
    ...
    FRPGDamageSpec BuildDamageSpec(const FHitResult& Hit) const;
    ERPGHitDirection ResolveHitDirectionAgainstActor(const AActor* TargetActor) const;
    UAnimInstance* GetOwnerAnimInstance() const;
    void ResolveDefaultTraceComponents();
    bool MontageUsesNotifyDrivenTraceWindow(const UAnimMontage* Montage) const;
